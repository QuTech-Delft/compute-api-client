...
from qi2_shared.client import Configuration, config, connect
...
def test_config(api_settings_mock: MagicMock, mocker: MockerFixture) -> None:
    # Arrange
    session = MagicMock()
    mocker.patch("qi2_shared.client.OauthDeviceSession", return_value=session)
    mocker.patch("qi2_shared.client.IdentityProvider")
