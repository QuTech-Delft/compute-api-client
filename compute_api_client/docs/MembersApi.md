...
> PageMember read_members_members_get(page=page, size=size, id=id, team_id=team_id, role=role, is_active=is_active, sort_by=sort_by, latest=latest)
...
async with compute_api_client.ApiClient(configuration) as api_client:
    ...
    try:
        # List members
        api_response = await api_instance.read_members_members_get(page=page, size=size, id=id, team_id=team_id, role=role, is_active=is_active, sort_by=sort_by, latest=latest)
